...
if(isset($_POST["product_code"]))
{
	...
	$statement = $mysqli_conn->prepare("SELECT product_name, product_price FROM shop_products WHERE product_code=? LIMIT 1");
...
if(isset($_POST["load_cart"]) && $_POST["load_cart"]==1)
{
	...
	if(isset($_SESSION["products"]) && count($_SESSION["products"])>0){ //if we have session variable
		...
		$cart_box .= "</ul>";
		$cart_box .= '<div class="cart-products-total">Total : '.$currency.sprintf("%01.2f",$total).' <u><a href="view_cart.php" title="Revisión del Carrito y Pagar ahora(Check-Out)">Pagar ahora</a></u></div>';
		die($cart_box); //exit and output content
	}else{
		die("Tu Carrito está Vacío"); //we have empty cart
	}
}
